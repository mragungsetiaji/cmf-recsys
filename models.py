...
def CMF(res_dir, train_user, train_item, valid_user, test_user,
    ...
    for iteration in range(max_iter):
        loss = 0
        tic = time.time()
        print("%d iteration\t(patience: %d)".format(iteration, count))
        ...
        print("Loss: %.5f Elpased: %.4fs Converge: %.6f Tr: %.5f Val: %.5f Te: %.5f".format(
